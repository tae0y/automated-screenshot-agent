...
def test_given_invalid_name_when_get_logger_invoked_then_should_return_logger(
    name
):
...
def test_given_valid_name_when_get_logger_invoked_then_should_return_logger(
    name
):
    logger = get_logger(name)
    assert isinstance(logger, logging.Logger)
...
@pytest.mark.parametrize(
    "name,level",
    [
        ("testLogger2", logging.ERROR),
        ("testLogger3", logging.INFO)
    ]
)
def test_given_level_when_get_logger_invoked_then_should_set_level(
    name, level
):
